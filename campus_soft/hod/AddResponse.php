...
<div id="page-wrapper">
            ...
            <div class="table-responsive">
              ...
              while($row =mysql_fetch_assoc($res))
              {
                  ...
                  $query1="select distinct(name),admissionno,classid from stud_details join current_class where admissionno='{$row['stud_id']}'";
...
if(isset($_POST["Send"]))
{   echo $date;
    $sql="update complaint1 set response='".$_POST['Response']."', res_time='".$date."',status=1 where id_com='".$id."'";
    $query=mysql_query($sql,$con);
    require_once("classes/class.phpmailer.php");
    // include the class name
    $mail="select guard_email from parent where parentid=(select parentid from parent_student where admissionno='{$stud}')";
    $res3= mysqli_query($conn,$mail); // find mailid
    if ($row=mysqli_fetch_assoc($res3)){
        $mail_to=$row['guard_email'];
    }
    //$mail_to = 'babu.amrutha23@gmail.com';
    ...
    $mail->IsHTML(true);
    $mail->Username = "ritsoft@rit.ac.in";
    $mail->Password = "ritsoft@123";
    $mail->SetFrom("ritsoft@rit.ac.in");
