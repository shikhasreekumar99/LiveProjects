...
if(isset($_POST['add_no']))
{
    ...
    $sql = "SELECT * FROM stud_details WHERE ($category='$add_no' AND status like 'on-going')";
    if($result = mysqli_query($link, $sql)){
        if(mysqli_num_rows($result) > 0){
            ...
            while($row = mysqli_fetch_array($result)){
				...
				echo "<td> <a target='_blank' href=ppldetails.php?id=".rawurlencode($row['admissionno']).">View </a></td>";
				echo "</tr>";
            }
			echo "</table></div> <input type='submit' name='excel' value='Download Excel' style=' height: 35px' id='excel_btn'>"; 
