    ...
    if($result = mysqli_query($link, $sql)){
        if(mysqli_num_rows($result) > 0){
            ...
            while($row = mysqli_fetch_array($result)){
				echo '<tr><td><img src="data:image/jpeg;base64,'.base64_encode( $row['image'] ).'" width="150" height="200"/></td></tr> ';
                echo "<tr>";
				 ...
				 echo "<tr><th>ROLL_NO</th><td>{$row['roll_no']}</td></tr>";
				 ...
				 echo "<tr><th>NO_CHANCE</th><td>{$row['no_chance']}</td></tr>";
				 ...
				 echo "<tr><th>BLOOD</th><td>{$row['blood']}</td></tr>";
                                 ...
                                 echo "<tr><th>GUARDIAN NAME</th><td>{$row['name_guardian']}</td></tr>";
